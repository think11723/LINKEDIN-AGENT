...
When the optional ``source`` dict is supplied, the Reviewer treats the
post as a source-inspired post and verifies that every factual claim
is supported by the source facts. A dedicated ``GROUNDING`` dimension
is added to the score breakdown; a low grounding score forces the
reviewer's improve-step to drop any unsupported claim rather than
re-state it.

The base prompt already covers LinkedIn-native formatting (no
Markdown, "Hashtags:" label forbidden, etc.). Source mode ADDS an
explicit grounding contract to the user message.
"""
...
class ReviewScores(BaseModel):
    ...
    # Phase 5 — source-aware grounding score. Only populated when the
    # reviewer was invoked with a ``source`` argument. A low score
    # forces an improve-step and surfaces a feedback message that
    # the operator can read in the draft.
    grounding: Optional[DimensionScore] = Field(
        default=None,
        description="Grounding to source facts (source mode only)",
    )
...
HASHTAGS: [improved hashtags]"""
    ...
    async def review(
        ...
        Args:
            ...
            source: Optional Phase-5 source context. When provided, the
                Reviewer scores a dedicated ``GROUNDING`` dimension and
                the improve-step is given the source facts so it can
                drop any claim that is not supported.

        Returns:
            ReviewResult with scores, feedback, and potentially improved post.
        """
        # Get comprehensive review. In source mode the review prompt
        # is augmented with the source facts so the model can score
        # grounding properly.
        scores, feedback, decision = await self._get_review(post, source=source)

        # Decide whether to improve based on overall score. In source
        # mode, a low GROUNDING score (< 6) also triggers an
        # improve-step even if overall is high — the persistence layer
        # would otherwise store a post that fabricates facts about
        # the source.
        needs_improvement = scores.overall < 8
        if source and scores.grounding is not None and scores.grounding.score < 6:
            needs_improvement = True
        if needs_improvement:
            final_post = await self._improve_post(post, source=source)
            was_improved = True
        else:
            final_post = post
            was_improved = False
    ...
    async def _get_review(
        ...
        post: LinkedInPost,
        source: Optional[Dict[str, Any]] = None,
    ) -> tuple[ReviewScores, str, Optional[ReviewDecision]]:
        """Get review scores, feedback, and decision for a post.

        Args:
            post: LinkedInPost to review.
            source: Optional Phase-5 source context. When provided, the
                user message is augmented with the source facts and
                a GROUNDING dimension is added to the score block.

        Returns:
            Tuple of ReviewScores, feedback string, and ReviewDecision.
        """
        prompt_source = self.review_prompt.format(
            title=post.title,
            content=post.content,
            hashtags=", ".join(post.hashtags)
        )
        if source:
            prompt_source = self._augment_review_prompt_with_source(
        ...
        response = await self.llm.generate_text(prompt_source, temperature=0.3)

        return self._parse_review_response(response.text)

    def _augment_review_prompt_with_source(
        self,
        base_prompt: str,
        source: Dict[str, Any],
    ) -> str:
        """Augment the standard review prompt with a source-grounding block.

        Adds:

        * a "GROUNDING" dimension in the score list (so the parser
          picks it up alongside the existing dimensions),
        * a SOURCE FACTS block at the end of the user message.
        """
        # Insert a GROUNDING dimension into the score list. The
        # parser below already understands dimension lines of the
        # form ``KEY: [score] | [explanation]``, so a single new
        # line at the right position is enough.
        grounding_line = (
            "GROUNDING: [1-10 score] | [explanation: how well the post's "
            "factual claims are supported by the SOURCE FACTS below; "
            "0 = fabricated/unsupported, 10 = every claim is grounded]"
        )
        if "GROUNDING:" not in base_prompt:
            base_prompt = base_prompt.replace(
                "AI_LIKE_WRITING: [score] | [explanation]",
                "AI_LIKE_WRITING: [score] | [explanation]\n" + grounding_line,
            )

        facts = source.get("source_summary") or ""
        if facts and len(facts) > 1500:
            facts = facts[:1500]
        key_points = source.get("key_points") or []
        key_points_block = "\n".join(f"- {str(p)[:300]}" for p in key_points[:8])
        source_block = (
            "\n═══════════════════════════════════════════════════════════════\n"
            "SOURCE FACTS (the post's factual claims MUST be supported by these):\n"
            f"Source Type: {source.get('source_type') or 'webpage'}\n"
            f"Source Title: {source.get('source_title') or ''}\n"
            f"Source URL: {source.get('source_url') or ''}\n"
            f"Source Summary: {facts}\n"
            f"Key Points:\n{key_points_block}\n"
        )
        if source.get('framing_hint'):
            source_block += (
                f"\nUSER FRAMING HINT: {source['framing_hint']}\n"
            )
        source_block += (
            "\nGROUNDING CONTRACT:\n"
            "- If the post contains a metric, scale, user count, ownership "
            "claim, performance number, or feature that is NOT in the "
            "SOURCE FACTS, score GROUNDING <= 4 and call it out in FEEDBACK.\n"
            "- If the post claims the user 'built', 'discovered', or "
            "'wrote' the source, score GROUNDING <= 4 unless the user "
            "explicitly said so in the USER FRAMING HINT.\n"
            "- If every factual claim in the post is supported by the "
            "SOURCE FACTS, score GROUNDING 9 or 10.\n"
        )
        return base_prompt + source_block
    ...
    def _parse_review_response(self, response: str) -> tuple[ReviewScores, str, Optional[ReviewDecision]]:
        ...
        # Phase 5 — grounding (only populated in source mode).
        grounding = None
        ...
        for line in lines:
            ...
            grounding = parse_dimension_score(line, "GROUNDING") or grounding
    ...
    async def _improve_post(
        ...
        Args:
            ...
            source: Optional Phase-5 source context. When provided, the
                improve-prompt is augmented with the source facts so
                the model can drop any unsupported claim rather than
                rewrite it.
        ...
        if source:
            prompt = self._augment_review_prompt_with_source(prompt, source)
